        ...
        if self.display_id > 0:  # show images in the browser using visdom
            ...
            if ncols > 0:        # show all the images in one visdom panel
                ...
                for label, image in visuals.items():
                    ...
                    images.append(image_numpy[0].transpose([2, 0, 1]))
                    idx += 1
                    if idx % ncols == 0:
                        label_html += '<tr>%s</tr>' % label_html_row
                        label_html_row = ''
                white_image = np.ones_like(image_numpy[0].transpose([2, 0, 1])) * 255
                while idx % ncols != 0:
                ...
                try:
                    for label, image in visuals.items():
                        image_numpy = util.tensor3d2im(image)
                        self.vis.image(image_numpy[0].transpose([2, 0, 1]), opts=dict(title=label),
